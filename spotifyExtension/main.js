(function(ext) {
    ...
    function getSpotifyAccessToken(tokenExpires) {
        return new Promise(function(resolve, reject) {
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'https://polar-hamlet-15645.herokuapp.com/token', true);
    
            xhr.onload = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    var response = JSON.parse(xhr.responseText);
                    response.expires_in += currentTime();
                    resolve(response);
                }
    ...
    function refreshTokenIfExpired(token) {
        return new Promise(function(resolve, reject) {
            if (currentTime() > token.expires_in) {
                getSpotifyAccessToken().then(function(response) {
                    resolve(response);
                });
            } else {
                resolve(token);
    ...
    function startWithToken(accessToken) {
        ...
        ext.search_songs = function(type, searchTerm, callback) {
            refreshTokenIfExpired(token).then(function(possibleNewToken) {
                $.ajax({
                    method: 'GET',
                    headers: {
                        'Authorization': 'Bearer ' + possibleNewToken.access_token
                    },
